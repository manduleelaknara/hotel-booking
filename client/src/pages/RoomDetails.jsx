...
const RoomDetails = () => {
    const { id } = useParams()
    const {rooms, getToken, axios, navigate, backendUrl} = useAppContext() 
        ...
        <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
            ...
            <div className='flex flex-col lg:flex-row mt-6 gap-6'>
                <div className='lg:w-1/2 w-full'>
                    <img src={`${backendUrl}${mainImage}`} alt="Room Image"
                        className='w-full rounded-xl shadow-lg object-cover' /> 
                </div>
                <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                    {room?.images.length > 1 && room.images.map((image, index) => (
                        <img onClick={() => setMainImage(image)}
                            key={index} src={`${backendUrl}${image}`} alt="Room Image"
            ...
            mx-auto mt-16 max-w-6xl'>
                ...
                md:items-center gap-4 md:gap-10 text-gray-500'>
                    ...
                    <div className='flex flex-col'>
                        <label htmlFor="guests" className='font-medium'>Guets</label>
            ...
            <div className='flex flex-col items-start gap-4'>
                ...
                hover:bg-primary-dull tansition-all cursor-pointer'>Contact Now</button>
            </div>
        </div>
    )
}
